...
class Projection:
  ...
  def Basis(self,U_Span):
    ...
    for i in range(0,min(U_reduced.shape[0],U_reduced.shape[1])):
        if((U_reduced[i,i] == 1)and(i not in indexes)): # this is a basis
            Basis.append(U_Span[:,i])
            indexes.append(i)
        else:
            for j in range(i,U_reduced.shape[1]):
                if((U_reduced[i,j] != 0)and(j not in indexes)):
                    Basis.append(U_Span[:,j])
                    indexes.append(j)
            for j in range(i,U_reduced.shape[0]):
                if((U_reduced[j,i] != 0)and(i not in indexes)):
                    Basis.append(U_Span[:,i])
                    indexes.append(i)
